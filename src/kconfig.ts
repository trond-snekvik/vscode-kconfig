...
export abstract class Scope {
    ...
    addScope(s: Scope): Scope {
        var existing = this.children.find(
            (c) => c instanceof Scope && c.id === s.id && c.file === s.file
        ) as Scope;
        if (existing) {
...
export class RootScope extends Scope {
    constructor(repo: Repository) {
        super(
            ...
            repo,
            0,
            new ParsedFile(repo, vscode.Uri.parse('commandline://'), {}, repo.rootScope),
...
export class ConfigEntry {
    ...
    isActive(ctx: EvalContext): boolean {
        return (
            this.dependencies.every((d) => resolveExpression(d, ctx)) && this.scope.evaluate(ctx)
        );
    }
}
...
export class Config {
            ...
            configs.push(
                ...e.implys
                    .filter(
                        (s) =>
                            s.name === name &&
                            !ctx.overrides.some((o) => o.config.name === name) &&
                            (!s.condition || s.condition.solve(ctx))
                    )
                    .map((s) => ctx.repo.configs[s.name])
                    .filter((c) => c !== undefined)
            );
    ...
    getRange(ctx: EvalContext): { min: number; max: number } {
        ...
        this.activeEntries(ctx).find((e) => {
            range = e.ranges.find(
                (r) => r.condition === undefined || r.condition.solve(ctx) === true
            );
            return range;
        });
    ...
    evaluate(ctx: EvalContext): ConfigValue {
        ...
        if (this.type === 'bool' || this.type === 'tristate') {
            ...
            if (
                this.entries[0].scope instanceof ChoiceScope &&
                this.entries[0].scope.choice.chosen(ctx) === this
            ) {
                return true;
            }
        }
...
export class ChoiceEntry extends ConfigEntry {
    ...
    chosen(ctx: EvalContext): Config | undefined {
        var c = this.choices.find((c) =>
            ctx.overrides.some((o) => o.config === c && c.resolveValueString(o.value))
        );
        if (c) {
            return c;
        }
...
export class Repository {
    ...
    constructor(diags: vscode.DiagnosticCollection) {
        this.configs = {};
        this.diags = diags;
        this.openEditors = vscode.window.visibleTextEditors
            .filter((e) => e.document.languageId === 'kconfig')
    ...
    activate(context: vscode.ExtensionContext) {
        context.subscriptions.push(
            vscode.window.onDidChangeVisibleTextEditors((e) => {
                ...
                var removed = this.openEditors.filter(
                    (old) => !newUris.some((uri) => uri.fsPath === old.fsPath)
                );
                var added = newUris.filter(
                    (newUri) => !this.openEditors.some((uri) => uri.fsPath === newUri.fsPath)
                );
    ...
    get files(): ParsedFile[] {
        // TODO: optimize to a managed dict?
        if (!this.root) {
            return [];
        }
                        ...
                        sum.concat(
                            diags.filter(
                                (d) =>
                                    !sum.some(
                                        (existing) =>
                                            existing.range.start.line === d.range.start.line
                                    )
                            )
                        ),
                    []
    ...
    onDidChange(uri: vscode.Uri, change?: vscode.TextDocumentChangeEvent) {
        ...
        if (vscode.debug.activeDebugSession) {
            console.log(
                `Handled changes to ${files.length} versions of ${uri.fsPath} in ${
    ...
    printStats() {
        ...
        console.log(
            `\tEmpty configs: ${this.configList.filter((c) => c.entries.length === 0).length}`
        );
        var entriesC = this.configList
            .map((c) => c.entries)
