...
export class PropFile {
            ...
            this.parseDiags.push(
                new vscode.Diagnostic(
                    new vscode.Range(
                        ...
                        valueOffset,
                        lineNumber,
                        valueOffset + valueMatch[2].length
                    ),
                    `Invalid value. Entry ${match[1]} is ${entry.type === 'int' ? 'an' : 'a'} ${
                        entry.type
                    }. ${hint}`,
                    vscode.DiagnosticSeverity.Error
                )
            ...
            this.parseDiags.push(
                new vscode.Diagnostic(
                    new vscode.Range(
                        ...
                        start,
                        thisLine.line,
                        start + trailing[1].trimRight().length
                    ),
    ...
    async parse() {
        ...
        this.conf = lines
            .map((l, i) => this.parseLine(l, i))
            .filter((c) => c !== undefined) as ConfigOverride[];
        this.updateDiags();
    }
    ...
    private getDependencyOverrides(missingDependency: string, ctx: EvalContext) {
        ...
        if (variables.length > 0 && variables.length < 4) {
            ...
            for (let bitmap = 0; bitmap < 1 << variables.length; bitmap++) {
                let replacements = variables.map((v, i) => {
                    return { name: v.name, value: bitmap & (1 << i) ? 'y' : 'n' };
                });
                ...
                if (makeExpr(replacedTokens).solve(ctx)) {
                    replacements.forEach((r) => {
                        ...
                        if (entryDep) {
                            entries.push(
                                ...this.getDependencyOverrides(entryDep, ctx).filter(
                                    (e) =>
                                        !entries.some(
                                            (existing) =>
                                                e.config.name === existing.config.name &&
    ...
    async lint() {
        ...
        var addRedundancyAction = (c: ConfigOverride, diag: vscode.Diagnostic) => {
            var action = new vscode.CodeAction(
                `Remove redundant entry CONFIG_${c.config.name}`,
        ...
        for (var i = 0; i < this.conf.length; i++) {
            ...
            if (c.config.type && ['int', 'hex'].includes(c.config.type)) {
                ...
                if (
                    (range.min !== undefined && override < range.min) ||
                    (range.max !== undefined && override > range.max)
                ) {
                    diags.push(
                        new vscode.Diagnostic(
                            line,
            ...
            if (missingDependencies.length) {
                let depText =
                    missingDependencies.length > 1
                        ? `dependencies`
                        : `dependency ${missingDependencies[0]}`;
                if (c.value === 'n') {
                    diag = new vscode.Diagnostic(
                        line,
                ...
                if (overrides.length) {
                    let action = new vscode.CodeAction(
                        `Fix ${overrides.length} missing ${
                            overrides.length > 1 ? 'dependencies' : 'dependency'
                        }`,
                        vscode.CodeActionKind.QuickFix
                    );
                    action.diagnostics = [diag];
            ...
            if (selector) {
                diag = new vscode.Diagnostic(
                    line,
                    `Entry ${c.config.name} is ${
                        c.value === 'n' ? 'ignored' : 'redundant'
                    } (Already selected by ${selector.name})`,
                    c.value === 'n'
                        ? vscode.DiagnosticSeverity.Warning
                        : vscode.DiagnosticSeverity.Hint
                );
                ...
                if (o && o.line !== undefined) {
                    ...
                    diag.relatedInformation = [
                        new vscode.DiagnosticRelatedInformation(
                            selector.entries[0].loc,
                            `Selected by ${selector.name}`
                        ),
                    ];
                }
                diag.tags = [vscode.DiagnosticTag.Unnecessary];
            ...
            if (override !== actualValue) {
                diags.push(
                    new vscode.Diagnostic(
                        line,
                        `Entry ${c.config.name} assigned value ${
                            c.value
                        }, but evaluated to ${c.config.toValueString(actualValue)}`,
                        vscode.DiagnosticSeverity.Warning
                    )
                );
                continue;
    ...
    onChange(e: vscode.TextDocumentChangeEvent) {
        ...
        this.lintDiags.forEach((diag) => {
            var diff = changes.reduce(
                (sum, change, _) =>
                    change.line <= diag.range.start.line ? sum + change.change : sum,
                0
            );
